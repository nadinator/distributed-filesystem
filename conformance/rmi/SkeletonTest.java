...
public class SkeletonTest extends Test {
  ...
  private void ensureSkeletonRuns() throws TestFailed {
    if (probe()) throw new TestFailed(
      "skeleton accepts connections before start"
    );
    ...
    if (!probe()) throw new TestFailed(
      "skeleton refuses connections after start"
    );
    ...
    if (probe()) throw new TestFailed(
      "skeleton accepts connections after stop"
    );
  }
  ...
  private void ensureClassRejected() throws TestFailed {
    try {
      Skeleton<Object> bad_skeleton = new Skeleton<Object>(
        Object.class,
        dummy_server
      );
      throw new TestFailed(
        "Skeleton(Class<T>, T) constructor has " + "accepted a class"
      );
    } catch (TestFailed e) {
      throw e;
    } catch (Error e) {} catch (Throwable t) {
      throw new TestFailed(
        "Skeleton(Class<T>, T) constructor threw " +
    ...
    try {
      ...
      throw new TestFailed(
        "Skeleton(Class<T>, T, InetSocketAddress) " +
  ...
  private void ensureNonRemoteInterfaceRejected() throws TestFailed {
    try {
      Skeleton<BadInterface> bad_skeleton = new Skeleton<BadInterface>(
        BadInterface.class,
        dummy_server
      );
      throw new TestFailed(
        "Skeleton(Class<T>, T) constructor has " +
  ...
  private void ensureNullPointerExceptions() throws TestFailed {
    ...
    try {
      Skeleton<TestInterface> bad_skeleton = new Skeleton<TestInterface>(
        null,
        server
      );
      throw new TestFailed(
        "Skeleton(Class<T>, T) constructor " +
    ...
    try {
      ...
      throw new TestFailed(
        "Skeleton(Class<T>, T, InetSocketAddress) " +
        "constructor threw an unexpected exception " +
        "when given null for first argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "Skeleton(Class<T>, T, InetSocketAddress) " +
        "constructor threw an unexpected exception " +
        "when given null for second argument",
        t
      );
    }
  }
