...
public class DeletionTest extends NamingTest {
  ...
  /** Test notice. */
  public static final String notice = "checking naming server delete method";
  /** Prerequisites. */
    ...
    for (
      ...
      try {
        ...
        throw new TestFailed(
          "unable to access " + delete_file + " for reading",
          t
        );
      }
    }
  ...
  private void testBadArguments() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "delete method threw unexpected exception " +
    ...
    try {
      ...
      if (result) {
        throw new TestFailed(
          "delete method accepted non-existent " + "file as argument"
        );
      } else {
        throw new TestFailed(
          "delete method returned false when " +
  ...
  protected void initialize() throws TestFailed {
    ...
    try {
      ...
      server1.start(
        registration_stub,
        new Path[] { delete_file, dummy_file },
        null
      );
    ...
    public synchronized boolean delete(Path path) {
      ...
      if (valid_path) {
        if (item_present) {
          ...
          if (is_file) hosts_file = false; else hosts_directory = false;

          return true;
        } else {
          failure(
            new TestFailed(
              "server asked to delete " +
              path +
      ...
      // fail the test.
      failure(new TestFailed("server asked to delete unexpected path " + path));
      return false;
    }
