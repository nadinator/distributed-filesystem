...
public class LockTest extends NamingTest {
  ...
  private void testSharing(
    ...
    task(
      ...
      first_path +
      " for " +
      accessType(first_exclusive) +
      " access and " +
      second_path +
      " for " +
      accessType(second_exclusive) +
  ...
  private void testExclusion(
    ...
    new Thread(new SecondExclusiveUser(second_path, second_exclusive)).start();
  ...
  private String accessType(boolean exclusive) {
    if (exclusive) return "exclusive"; else return "shared";
  }
  ...
  private void testBadPaths() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "lock method threw unexpected exception " +
    ...
    try {
      ...
      throw new TestFailed(
        "unlock method accepted null for path " + "argument"
      );
    } catch (TestFailed e) {
      throw e;
    } catch (NullPointerException e) {} catch (Throwable t) {
      throw new TestFailed(
        "unlock method threw unexpected exception " +
        "when given null for path argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "lock method threw unexpected exception " + "when given bad path",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "unlock method threw unexpected exception " + "when given bad path",
        t
      );
    }
  }
