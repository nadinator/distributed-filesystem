...
public class CreationTest extends NamingTest {
  ...
  private void testFileCreation(Path file) throws TestFailed {
    ...
    if (!result) throw new TestFailed("unable to create new file");
    ...
    try {
      ...
      throw new TestFailed(
        "unexpected exception when checking file " + file,
        t
      );
    }
    ...
    if (result) {
      throw new TestFailed(
        "new file " + file + " reported as a " + "directory"
      );
    }
    ...
    task(
      "waiting for command to create " + file + " on the storage " + "server"
    );
  ...
  private void testDirectoryCreation(Path directory) throws TestFailed {
    ...
    if (!result) throw new TestFailed(
      "unable to create new directory " + directory
    );
    ...
    try {
      ...
      throw new TestFailed(
        "naming server failed to create directory " + directory,
        e
      );
    } catch (Throwable t) {
      throw new TestFailed(
        "unexpected exception when checking " + "directory " + directory,
        t
      );
    }
    ...
    if (!result) {
      throw new TestFailed(
        "new directory " + directory + " reported " + "as a file"
      );
    }
  }
  ...
  private void testCreateDirectoryArguments() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "createDirectory threw unexpected exception " +
    ...
    try {
      ...
      throw new TestFailed(
        "createDirectory accepted new directory " +
    ...
    if (result) {
      throw new TestFailed(
        "createDirectory accepted a path to an " + "existing directory"
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "createDirectory threw unexpected exception " +
        "when given a path to a file",
        t
      );
    }

    if (result) throw new TestFailed(
      "createDirectory accepted a path to a file"
    );
    ...
    try {
      ...
      throw new TestFailed(
        "createDirectory threw unexpected exception " +
        "when given the root directory",
        t
      );
    }

    if (result) throw new TestFailed(
      "createDirectory accepted the root directory"
    );
  }
  ...
  private void testCreateFileArguments() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "createFile threw unexpected exception when " +
        "given null as argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "createFile accepted path to new file whose " +
    ...
    try {
      ...
      throw new TestFailed(
        "createFile accepted new file whose parent " + "directory is a file"
      );
    } catch (TestFailed e) {
      throw e;
    } catch (FileNotFoundException e) {} catch (Throwable t) {
      throw new TestFailed(
        "createFile threw unexpected exception " +
    ...
    if (result) {
      throw new TestFailed(
        "createFile accepted a path to an existing" + "file"
      );
    }
    ...
    if (result) throw new TestFailed(
      "createFile accepted a path to a directory"
    );
    ...
    if (result) throw new TestFailed("createFile accepted the root directory");
  }
  ...
  private class CreationTestStorageServer extends TestStorageServer {
    ...
    public boolean create(Path file) {
      ...
      if (expect_file == null) super.create(file); else {
        if (file == null) {
          failure(
            new TestFailed("create method called with null " + "as argument")
          );
        ...
        if (!file.equals(expect_file)) {
          failure(
            new TestFailed(
              "create method called with wrong " +
              "argument: expected " +
              expect_file +
              ", but got " +
              file
