...
public class StubRetrievalTest extends NamingTest {
  ...
  private void checkStub(Path path, Storage expected_stub) throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "unable to retrieve storage server stub for " + path,
        t
      );
    }

    if (stub == null) throw new TestFailed(
      "received null instead of stub for " + path
    );

    // Check that the stub received is equal to the stub expected.
    if (!stub.equals(expected_stub)) throw new TestFailed(
      "received wrong stub for " + path
    );
  }
  ...
  private void checkArguments() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "getStorage threw unexpected exception when " +
    ...
    try {
      ...
      throw new TestFailed(
        "getStorage accepted path to non-existent " + "file as argument"
      );
    } catch (TestFailed e) {
      throw e;
      ...
      throw new TestFailed(
        "getStorage threw unexpected exception when " +
        "given a non-existent file as argument",
        t
      );
    }
  ...
  protected void initialize() throws TestFailed {
    ...
    try {
      server1_stub =
        server1.start(registration_stub, new Path[] { file1, file2 }, null);
      server2_stub =
        server2.start(
          registration_stub,
