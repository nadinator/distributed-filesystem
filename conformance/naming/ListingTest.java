...
public class ListingTest extends NamingTest {
  ...
  private void checkDirectoryListing(
    ...
    if (listing == null) {
      throw new TestFailed(
        "listing directory " + path + " resulted in " + "null"
      );
    }
    ...
    for (
      int child_index = 0;
      child_index < expected_children.length;
      ++child_index
    ) {
      ...
      try {
        ...
        throw new TestFailed(
          "unable to determine if " + child_path + " is a directory",
          t
        );
      }
      ...
      if (is_directory && !expected_kinds[child_index]) {
        throw new TestFailed(
          child_path + " is reported as a " + "directory but is in fact a file"
        );
      }
      if (!is_directory && expected_kinds[child_index]) {
        throw new TestFailed(
          child_path + " is reported as a file " + "but is in fact a directory"
        );
      }
    }
  }
  ...
  private void checkArguments() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "isDirectory threw unexpected exception " +
    ...
    try {
      ...
      throw new TestFailed(
        "list threw unexpected exception when given " + "null as argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "isDirectory accepted a non-existent path " + "as argument"
      );
    } catch (TestFailed e) {
      throw e;
    ...
    try {
      ...
      throw new TestFailed(
        "list threw unexpected exception when given " +
        "a non-existent path as argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "list threw unexpected exception when given " + "a file as argument",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "unable to determine if the root is a " + "directory",
        t
      );
    }

    if (!root_is_directory) throw new TestFailed(
      "root is not reported as a directory"
    );
  }
