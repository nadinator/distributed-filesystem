  ...
  private void testMerging() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "unable to register storage server with " + "naming server",
        t
      );
    }
    ...
    Path[] server2_files = new Path[] {
      new Path("/file"),
      new Path("/directory/file"),
      new Path("/another_directory/another_file"),
    };
    Path[] server2_delete_files = new Path[] {
      new Path("/file"),
      new Path("/directory/file"),
    };
    ...
    try {
      synchronized (this) {
        server2.start(registration_stub, server2_files, server2_delete_files);
      }
    } catch (TestFailed e) {
      throw new TestFailed(
        "naming server did not command deletion " +
        "of the expected files when checking " +
        "regular files"
      );
    } catch (Throwable t) {
      throw new TestFailed(
        "unable to register second storage server " + "with naming server",
        t
      );
    }
    ...
    try {
      synchronized (this) {
        server3.start(registration_stub, server3_files, server3_delete_files);
      }
    } catch (TestFailed e) {
      throw new TestFailed(
        "naming server did not command deletion " +
      ...
      throw new TestFailed(
        "unable to register third storange server " + "with naming server",
        t
      );
    }
    ...
    try {
      synchronized (this) {
        server4.start(registration_stub, server4_files, server4_delete_files);
      }
    } catch (Throwable t) {
      throw new TestFailed(
        "naming server did not silently ignore " +
        "request to add root directory as a file",
        t
      );
    }
  }
  ...
  private class UnsuccessfulStorageServer extends TestStorageServer {
    ...
    synchronized void testDuplicateRegistration() throws TestFailed {
      ...
      try {
        ...
        throw new TestFailed(
          "unable to start skeletons for " + "duplicate registration test",
          t
        );
      }
      ...
      try {
        registration_stub.register(client_stub, command_stub, new Path[0]);
      } catch (IllegalStateException e) {
        throw new TestFailed(
          "storage server reported as already " +
          "registered during initial registration " +
          "in duplicate registration test: " +
          "perhaps it was mistakenly registered " +
          "during the invalid reference test",
          e
        );
      } catch (Throwable t) {
        throw new TestFailed(
          "unable to perform initial registration " +
      ...
      try {
        registration_stub.register(client_stub, command_stub, new Path[0]);
        throw new TestFailed(
          "naming server accepted duplicate " + "registration"
        );
      } catch (TestFailed e) {
        throw e;
      } catch (IllegalStateException e) {} catch (Throwable t) {
        throw new TestFailed(
          "register method threw unexpected " +
    ...
    synchronized void testNullPointers() throws TestFailed {
      ...
      try {
        ...
        throw new TestFailed(
          "unable to start skeletons for invalid " + "reference test",
          t
        );
      }
      ...
      try {
        ...
        throw new TestFailed(
          "register method accepted null for " +
        ...
        throw new TestFailed(
          ...
          "server client interface argument",
          t
        );
      }
      ...
      try {
        ...
        throw new TestFailed(
          "register method threw unexpected " +
          "exception when given null for storage " +
          "server command interface argument",
          t
        );
      }
      ...
      try {
        ...
        throw new TestFailed(
          "register method accepted null for " + "path argument"
        );
      } catch (TestFailed e) {
        throw e;
      } catch (NullPointerException e) {} catch (Throwable t) {
