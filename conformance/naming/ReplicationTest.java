...
public class ReplicationTest extends NamingTest {
  ...
  protected void perform() throws TestFailed {
    ...
    for (access_counter = 0; access_counter < ACCESS_COUNT; ++access_counter) {
      try {
        ...
        throw new TestFailed(
          "unable to lock " + replicate_file + " for reading",
          t
        );
      }
    ...
    if (invalidated) throw new TestFailed(
      replicate_file + " invalidated prematurely"
    );
    ...
    try {
      ...
      throw new TestFailed(
        "unable to lock " + replicate_file + " for " + "writing",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed("unable to unlock " + replicate_file, t); // Failing here
    }
  ...
  protected void initialize() throws TestFailed {
    ...
    try {
      ...
      hosting_stub =
        hosting_server.start(
          registration_stub,
          new Path[] { replicate_file },
          null
        );
  ...
  private class MirrorStorageServer extends InvalidationStorageServer {
    ...
    public boolean copy(Path file, Storage server) {
      ...
      if (server == null) {
        failure(
          new TestFailed("server stub argument to copy method " + "null")
        );
        return false;
      }
      ...
      if (!server.equals(hosting_stub)) {
        failure(
          new TestFailed(
            "naming server provided the wrong " + "storage server stub"
          )
        );
        return false;
      }
      ...
      synchronized (ReplicationTest.this) {
        if (replicated) {
          failure(new TestFailed("duplicate request to copy " + file));
          return false;
        }
  ...
  private class InvalidationStorageServer extends TestStorageServer {
    ...
    public boolean delete(Path path) {
      ...
      if (!path.equals(replicate_file)) {
        failure(
          new TestFailed(
            "naming server requested deletion of " +
            path +
            ", but deletion of " +
            replicate_file +
            " is expected"
      ...
      synchronized (ReplicationTest.this) {
        if (invalidated) {
          failure(
            new TestFailed(
              "more than one server requested " + "delete " + replicate_file
            )
          );
        }
