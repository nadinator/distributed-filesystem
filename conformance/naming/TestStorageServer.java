...
class TestStorageServer implements Storage, Command {
  ...
  synchronized Storage start(
    ...
    Path[] delete_files = naming_server.register(
      client_stub,
      command_stub,
      offer_files
    );
    ...
    if (expect_files != null) {
      if (!TestUtil.sameElements(delete_files, expect_files)) {
        throw new TestFailed(
          "naming server did not command deletion " + "of the expected files"
        );
      }
    }
  ...
  public long size(Path file) {
    test.failure(
      new TestFailed("unexpected call to size method in " + "storage server")
    );
  ...
  public byte[] read(Path file, long offset, int length) {
    test.failure(
      new TestFailed("unexpected call to read method in " + "storage server")
    );
  ...
  public void write(Path file, long offset, byte[] data) {
    test.failure(
      new TestFailed("unexpected call to write method in " + "storage server")
    );
  ...
  public boolean create(Path file) {
    test.failure(
      new TestFailed("unexpected call to create method in " + "storage server")
    );
  ...
  public boolean delete(Path file) {
    test.failure(
      new TestFailed("unexpected call to delete method in " + "storage server")
    );
  ...
  public boolean copy(Path file, Storage server) {
    test.failure(
      new TestFailed("unexpected call to copy method in " + "storage server")
    );
  ...
  private class StorageSkeleton extends Skeleton<Storage> {
    ...
    protected boolean listen_error(Exception e) {
      test.failure(
        new TestFailed(
          "caught exception in storage " + "skeleton listening thread",
          e
        )
      );
    ...
    protected void service_error(RMIException e) {
      test.failure(
        new TestFailed(
          "caught exception in storage " + "skeleton service thread",
          e
        )
      );
    }
  ...
  private class CommandSkeleton extends Skeleton<Command> {
    ...
    protected boolean listen_error(Exception e) {
      test.failure(
        new TestFailed(
          "caught exception in command " + "skeleton listening thread",
          e
        )
      );
    ...
    protected void service_error(RMIException e) {
      test.failure(
        new TestFailed(
          "caught exception in command " + "skeleton service thread",
          e
        )
      );
    }
