...
class TestNamingServer implements naming.Registration {
  ...
  @Override
  public synchronized Path[] register(
    Storage client_stub,
    ...
    if (client_stub == null) {
      test.failure(
        new TestFailed(
          "storage server client interface " + "null during registration"
        )
      );
    }
    ...
    if (command_stub == null) {
      test.failure(
        new TestFailed(
          "storage server command interface " + "null during registration"
        )
      );
    }
    ...
    if (expect_files != null) {
      if (!TestUtil.sameElements(files, expect_files)) {
        test.failure(
          new TestFailed("received wrong file list during " + "registration")
        );
      }
    }
    ...
    if (delete_files != null) return delete_files; else return new Path[0];
  }
  ...
  private class TestSkeleton extends Skeleton<Registration> {
    ...
    protected boolean listen_error(Exception e) {
      test.failure(
        new TestFailed(
          "caught exception in naming server " + "listening thread",
          e
        )
      );
    ...
    protected void service_error(RMIException e) {
      test.failure(
        new TestFailed(
          "caught exception in naming server " + "service thread",
          e
        )
      );
    }
