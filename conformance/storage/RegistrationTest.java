...
public class RegistrationTest extends StorageTest {
  ...
  protected void perform() throws TestFailed {
    ...
    Path[] remaining_files = new Path[] {
      new Path("/subdirectory/file2"),
    ...
    if (!TestUtil.sameElements(listed, remaining_files)) {
      throw new TestFailed(
        "storage server did not remove correct " + "files after registration"
      );
    }
    ...
    if (pruned.exists()) {
      throw new TestFailed(
        "storage server did not prune directories " + "that contain no files"
      );
    }
  }
}
