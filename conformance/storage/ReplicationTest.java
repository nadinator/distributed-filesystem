...
public class ReplicationTest extends StorageTest {
  ...
  private void testCreation() throws TestFailed {
    ...
    try {
      if (!command_stub.copy(create_path, second_stub)) {
        throw new TestFailed(
          "unable to create new file by " + "replication: copy returned false"
        );
      }
    } catch (TestFailed e) {
      throw e;
    ...
    if (resulting_size != (long) data.length) throw new TestFailed(
      "created file has incorrect size"
    );

    try {
      resulting_data = client_stub.read(create_path, 0, (int) resulting_size);
    } catch (Throwable t) {
      throw new TestFailed("unable to retrieve contents of created file", t);
    }

    if (!Arrays.equals(resulting_data, data)) throw new TestFailed(
      "created file has incorrect contents"
    );
  }
  ...
  private void testReplacement() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "unable to write data to file to be " +
    ...
    try {
      ...
      throw new TestFailed(
        "unable to write data to replacing file on " + "second storage server",
        t
      );
    }
    ...
    try {
      if (!command_stub.copy(replace_path, second_stub)) {
        throw new TestFailed(
          "unable to replace file by " + "replication: copy returned false"
        );
      }
    } catch (TestFailed e) {
      throw e;
    ...
    if (resulting_size != (long) new_data.length) throw new TestFailed(
      "replaced file has incorrect size"
    );

    try {
      resulting_data = client_stub.read(replace_path, 0, (int) resulting_size);
    } catch (Throwable t) {
      throw new TestFailed("unable to retrieve contents of replaced file", t);
    }

    if (!Arrays.equals(resulting_data, new_data)) throw new TestFailed(
      "replaced file has incorrect contents"
    );
  }
  ...
  private void testBadValues() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed(
        "copy accepted null for peer storage server " + "interface"
      );
    } catch (TestFailed e) {
      throw e;
    } catch (NullPointerException e) {} catch (Throwable t) {
      throw new TestFailed(
        "copy threw unexpected exception when given " +
        "null for peer storage server interface",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "copy threw unexpected exception when given " + "null for path",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "copy threw unexpected exception when given " + "missing file",
        t
      );
    }
    ...
    try {
      ...
      throw new TestFailed(
        "copy threw unexpected exception when given " + "directory",
        t
      );
    }
  }
  ...
  protected void initialize() throws TestFailed {
    ...
    try {
      ...
      throw new TestFailed("unable to create second temporary directory", t);
    }
    ...
    try {
      ...
      throw new TestFailed(
        "unable to add file to second temporary " + "directory",
        t
      );
    }
    ...
    try {
      second_server = new StorageServer(second_directory.root());
    } catch (Throwable t) {
