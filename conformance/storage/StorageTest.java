...
abstract class StorageTest extends Test {
  ...
  protected void initialize() throws TestFailed {
    ...
    try {
      if (test_files != null) {
        for (String[] path : test_files) directory.add(path);
      }
    } catch (Throwable t) {
    ...
    if (test_files != null) {
      ...
      for (int index = 0; index < test_files.length; ++index) {
        ...
        for (String component : test_files[index]) current_path =
          new Path(current_path, component);
    ...
    try {
      naming_server = new TestNamingServer(this);
