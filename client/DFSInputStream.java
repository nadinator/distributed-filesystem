...
public class DFSInputStream extends InputStream {
  ...
  public int read(byte[] buffer, int buffer_offset, int read_length)
    ...
    if (closed) {
      throw new IOException(
        "distributed filesystem input stream " + "already closed"
      );
    }

    // Check that all the arguments are valid.
    if (buffer == null) throw new NullPointerException(
      "buffer array argument is null"
    );

    if (buffer_offset < 0) throw new IndexOutOfBoundsException(
      "buffer offset is negative"
    );

    if (read_length < 0) throw new IndexOutOfBoundsException(
      "read length is negative"
    );

    if (
      (buffer_offset + read_length) > buffer.length
    ...
    // file.
    if (read_length > (length - offset)) read_length = (int) (length - offset);
    ...
    for (int index = 0; index < read_length; ++index) buffer[buffer_offset +
      index] =
      result[index];
  ...
  public int available() throws IOException {
    ...
    if (length - offset > Integer.MAX_VALUE) return Integer.MAX_VALUE;
