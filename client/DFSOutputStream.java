  ...
  public void write(byte[] buffer, int buffer_offset, int write_length)
    ...
    if (closed) {
      throw new IOException(
        "distributed filesystem output stream " + "already closed"
      );
    }
    ...
    if (write_length < 0) throw new IndexOutOfBoundsException(
      "write length is negative"
    );

    if (
      (buffer_offset + write_length) > buffer.length
    ...
    if ((buffer_offset == 0) && (buffer.length == write_length)) data =
      buffer; else {
      data = new byte[write_length];

      for (int index = 0; index < write_length; ++index) data[index] =
