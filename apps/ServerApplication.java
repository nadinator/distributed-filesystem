...
abstract class ServerApplication extends Application {
  ...
  protected void run(String[] arguments) {
    ...
    System.out.println(
      "send EOF on standard input (Ctrl+D or Ctrl+Z, " +
    ...
    if (stop_cause == null) {
      ...
      System.err.println(
        serverType() + " server stopped due to an " + "exception"
      );
  ...
  protected void startTerminationTimer() {
    termination_timeout_timer.schedule(
      new TerminationTimeoutTask(),
      TERMINATION_TIMEOUT
    );
  }
  ...
  private class EOFThread implements Runnable {
    ...
    public void run() {
      ...
      // server.
      try {
        while (System.in.read() != -1) continue;
      } catch (IOException e) {}
  ...
  private class TerminationTimeoutTask extends TimerTask {
    ...
    public void run() {
      ...
      synchronized (ServerApplication.this) {
        ...
        System.err.println(
          serverType() + " server timed out while " + "stopping"
        );
        System.exit(EXIT_FAILURE);
      }
    }
