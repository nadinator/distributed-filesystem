  ...
  private void list(String remote_path, boolean show_path)
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot lock " + object + ": " + t.getMessage()
      );
    }

    try {
      if (naming_server.isDirectory(object.path)) components =
        naming_server.list(object.path); else components =
        new String[] { object.path.last() };
    } catch (Throwable t) {
      throw new ApplicationFailure(
        "cannot list " + object + ": " + t.getMessage()
      );
    } finally {
    ...
    if (show_path) System.out.println(remote_path + ":");

    for (int index = 0; index < components.length; ++index) {
      if (show_path) System.out.print("\t");
