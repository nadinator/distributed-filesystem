  ...
  public void coreLogic(String[] arguments) throws ApplicationFailure {
    if (arguments.length != 2) {
      throw new ApplicationFailure(
        "usage: put source_file " + "destination_file"
      );
    }
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot parse destination path: " + e.getMessage()
      );
    }

    // The source must refer to an existing file.
    if (!source.exists()) throw new ApplicationFailure(
    ...
    if (destination.path.isRoot()) path_to_lock =
      destination.path; else path_to_lock = destination.path.parent();
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot lock " + path_to_lock + ": " + t.getMessage()
      );
    }
    ...
    try {
      ...
      try {
        if (naming_server.isDirectory(destination.path)) {
          ...
          if (filename.length() == 0) throw new ApplicationFailure(
      ...
      while (bytes_remaining > 0) {
        ...
        if (bytes_remaining < BLOCK_SIZE) bytes_to_transfer =
          (int) bytes_remaining;
        ...
        while (bytes_loaded < bytes_to_transfer) {
          ...
          if (bytes_read <= 0) throw new EOFException("unexpected end of file");
      ...
      throw new ApplicationFailure(
        "cannot transfer " + source + ": " + t.getMessage()
      );
    } finally {
