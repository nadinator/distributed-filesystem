  ...
  public void coreLogic(String[] arguments) throws ApplicationFailure {
    if (arguments.length != 2) {
      throw new ApplicationFailure(
        "usage: get source_file " + "destination_file"
      );
    }
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot parse source path: " + e.getMessage()
      );
    }

    // Make sure the source file is not the root directory.
    if (source.path.isRoot()) throw new ApplicationFailure(
    ...
    if (destination.isDirectory()) destination =
      new File(destination, source.path.last());
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot lock " + source + ": " + t.getMessage()
      );
    }
    ...
    try {
      ...
      while (bytes_remaining > 0) {
        ...
        if (bytes_to_transfer > BLOCK_SIZE) bytes_to_transfer = BLOCK_SIZE;
