...
public class Touch extends ClientApplication {
  ...
  private void touch(String remote_file) throws ApplicationFailure {
    ...
    if (file.path.isRoot()) throw new ApplicationFailure(
      "cannot touch root directory"
    );
    ...
    try {
      ...
      throw new ApplicationFailure(
        "cannot lock " + file.parent() + ": " + t.getMessage()
      );
    }
    ...
    try {
      ...
      if (naming_server.isDirectory(file.path)) throw new ApplicationFailure(
        file + " is a directory"
      );
    } catch (ApplicationFailure e) {
      throw e;
      ...
      throw new ApplicationFailure(
        "cannot access " + file + ": " + t.getMessage()
      );
    } finally {
