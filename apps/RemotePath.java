  ...
  private String parseHostname(String hostname) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
        "cannot retrieve value of " + hostname_variable
      );
    }

    if ((hostname == null) || (hostname.length() == 0)) {
      throw new IllegalArgumentException(
        "no hostname is given and no " + "current hostname is defined"
      );
    }
  ...
  private Path parseRemotePath(String raw_path) {
    ...
    if (raw_path.indexOf(separator) == 0) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
          "cannot retrieve value of " + directory_variable
        );
      }

      // If DFSCWD is not set, use / as a default value.
      if (working_directory == null) working_directory = separator;
    ...
    while (cursor_position < raw_path.length()) {
      ...
      if (end_of_component == -1) end_of_component = raw_path.length();
      ...
      // path.
      if (component.equals(current_directory)) continue;
      ...
      if (component.equals(parent_directory)) {
        if (current_path.isRoot()) {
          throw new IllegalArgumentException(
            "path component " + "refers to parent of " + "root directory"
          );
        }
