...
public class TemporaryDirectoryTest extends Test {
  ...
  protected void perform() throws TestFailed {
    ...
    if (root.equals(second_root)) {
      throw new TestFailed(
        "temporary directories created with the " + "same path"
      );
    }
    ...
    if (second_root.exists()) throw new TestFailed(
    ...
    if (root.exists()) throw new TestFailed("temporary directory not removed");
  }
  ...
  protected void clean() {
    ...
    if (second_directory != null) second_directory.remove();
  }
}
