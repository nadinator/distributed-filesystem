...
public class Series implements Serializable {
  ...
  public Series(Class<? extends Test>[] tests) {
    ...
    if (tests == null) throw new NullPointerException("tests array is null");
    ...
    for (int test_index = 0; test_index < tests.length; ++test_index) {
      if (tests[test_index] == null) {
        throw new NullPointerException(
          "test " + test_index + " in " + "the test array is null"
        );
      }
    }
  ...
  public SeriesReport run(int timeout, PrintStream stream) {
    ...
    // Test queue.
    List<Class<? extends Test>> test_queue = new LinkedList<Class<? extends Test>>(
      Arrays.asList(tests)
    );
    ...
    main_loop:while (progress) {
      ...
      while (test_iterator.hasNext()) {
        ...
        if (ready(test_class, successful_tests)) {
          ...
          // Run the test.
          TestReport result = run(test_class, timeout_timer, timeout);
          ...
          // method.
          if (result.fatal()) break main_loop;
        }
      }
    }
    ...
    if (stream != null) {
      if (
        (successful_tests.size() > 0) || (failure_reports.size() > 0)
      ) stream.println();
    }
  ...
  /** Runs a single test.
     ...
     */
  private TestReport run(
    Class<? extends Test> test_class,
    Timer timeout_timer,
    int timeout
  ) {
    ...
    // not be modified.
    if (state.test == null) return new TestReport(
      test_class,
      state.cause,
      null
    );
    ...
    // threads.
    return new TestReport(test_class, state.cause, state.cleanup_stop_cause);
  }
  ...
  static class TestState {
    ...
    synchronized void stop(Throwable cause) {
      ...
      if ((cause != null) && (task != null)) cause =
        new FailedDuringTask(task, cause);
    ...
    synchronized void stopCleanup(FatalError cause) {
      if (!stopped) {
        stop(
          new FatalError(
            "cleanupSuccess or cleanupFailure method " +
  ...
  private static class TestThread implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        state.stop(
          new FatalError(
            "test class does not have a public " + "no-argument constructor",
            e
          )
        );
        return;
        ...
        state.stop(
          new FatalError(
            "access denied to test class " + "no-argument constructor",
            e
          )
        );
        return;
        ...
        state.stop(
          new FatalError("unable to retrieve test " + "constructor", t)
        );
        return;
      }
      ...
      try {
        ...
        if ((cause != null) && (cause instanceof TestFailed)) state.stop(
          (TestFailed) cause
        ); else {
          state.stop(
            new TestFailed(
              "test constructor threw " + "exception",
        ...
        state.stop(
          new TestFailed("unable to invoke test class " + "constructor", t)
        );
        return;
      }
  ...
  private static class CleanupThread implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        state.stopCleanup(new FatalError("clean method threw exception", t));
      }
  ...
  private static boolean ready(
    ...
    Class<? extends Test>[] prerequisites = staticField(
      test_class,
      Class[].class,
      "prerequisites",
      new Class[] {}
    ...
    for (Class<? extends Test> prerequisite : Arrays.asList(prerequisites)) {
      if (!successful_tests.contains(prerequisite)) return false;
    }
  ...
  /** Retrieves the value of a static field in a class.
     ...
     */
  private static <T> T staticField(
    Class<?> c,
    Class<T> field_type,
    String name,
    T default_value
    ...
    try {
      ...
      if (!Modifier.isStatic(field.getModifiers())) {
        throw new NoSuchElementException(name + " is not static in " + c);
      }

      if (!Modifier.isFinal(field.getModifiers())) {
        throw new NoSuchElementException(name + " is not final in " + c);
      }
      ...
      if (unchecked_value == null) throw new NullPointerException(
        name + " in " + c + " is null"
      );
